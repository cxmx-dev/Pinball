...
(function testHall1Refinements() {
  ...
  assert.strictEqual(dashes.length, 9, 'plunger-hall running lights restored to 9');
  var ys = dashes.map(function (d) { return d.y; });
  var yMin = Math.min.apply(null, ys);
  var yMax = Math.max.apply(null, ys);
  assert(yMin >= 490 && yMin <= 520, 'dash yTop ~500 (got ' + yMin + ')');
  assert(yMax >= 700, 'dash yBot stays near plunger (got ' + yMax + ')');
  dashes.forEach(function (d) {
    assert(d.y >= 360, 'no hall dash enters merge/U (y=' + d.y + ')');
  ...
  assert(renSrc.indexOf('if (d.y < 360)') !== -1, 'draw skip is y<360');
