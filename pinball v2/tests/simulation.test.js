...
(function testCopperMergePocketUnsticks() {
  ...
  assert(sim.HABITRAIL_ASSIST === 0, 'HABITRAIL_ASSIST stays 0');
  assert(sim.BOINGER_B_X === 374 && sim.BOINGER_B_Y === 740, 'B at 374,740 (down-right outlane)');
...
(function testTop2SausageJoinAndGate() {
  ...
  assert(Math.abs(cyanEnd.x2 - 280) < 2 && Math.abs(cyanEnd.y2 - 8) < 3, 'cyan outer ends at the join (280,8)');
  var copperJoin = outer[outer.length - 1];
  assert(Math.abs(copperJoin.x2 - 280) < 2 && Math.abs(copperJoin.y2 - 8) < 3, 'copper outer ends at the join (280,8)');
...
(function testNeed1UpperFlipperRubberTriangleLodge() {
  ...
  assert(mid500 && mid500.x === 300 && mid500.y === 478, '500 stays mid-field');
...
(function testPop1TriangleAndDumbbell() {
  ...
  assert(idxSrc.indexOf('?v=boing2') !== -1, 'cache bust boing2');
...
(function testFlip1SweptFlipperBlocksAndCenterHole() {
  ...
  assert.strictEqual(sim.BOINGER_B_X, 374, 'B nudged 24px left to 374');
  assert.strictEqual(sim.BOINGER_B_Y, 740, 'B y stays 740');
