...
(function testSideHullFillers() {
  ...
  assert(left.segments[0].x1 === 48 && left.segments[0].y1 === 586, 'left filler slid up the rail');
  assert(left.guides.some(function (s) { return s.x2 === 75 && s.y2 === 651; }), 'left bulge after CW rotate');
  assert(right.segments[0].x1 === 392 && right.segments[0].y1 === 609, 'right filler longer up the plunger wall');
  ...
  assert(fillerWalls.some(function (w) { return w.x1 === 48 && w.y1 === 586; }), 'left filler wall moved up');
  ...
  assert(Math.min.apply(null, rys) >= 580 && Math.min.apply(null, rys) <= 630, 'right filler longer upward');
