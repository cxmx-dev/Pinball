...
(function testBoingerPairAlternateAndKick() {
  ...
  assert(list && list.length === 2, 'two boingers');
  var a = list[0];
  var b = list[1];
  ...
  assert.strictEqual(b.x, 258);
  assert.strictEqual(b.y, 690);
  ...
  assert(a.up && !b.up, 'A up / B down at t=0 (inverted phase)');
  sim.stepBoinger(state, 2.9);
  assert(a.up && !b.up, 'A still up / B down at 2.9s');
  sim.stepBoinger(state, 0.2);
  assert(!a.up && b.up, 'A down / B up after 3.1s');
  sim.stepBoinger(state, 1.5);
  assert(a.up && !b.up, 'A up / B down after full 4.5s cycle');
  state.ball.inPlay = true;
  a.up = false;
  a.pop = 0;
  b.up = false;
