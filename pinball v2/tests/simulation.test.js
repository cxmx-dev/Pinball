...
(function testFillersShiftedTowardRails() {
  ...
  assert(lg[lg.length - 1].y2 >= 68 && lg[lg.length - 1].y2 <= 82, 'cyan inner floor stays coastable under the flatter outer');
...
(function testPop1TriangleAndDumbbell() {
  ...
  assert(renSrcPop.indexOf('var blend = 26') !== -1, 'join overlap 26px at x=280');
  ...
  assert(idxSrc.indexOf('?v=shoe3') !== -1, 'cache bust shoe3');
...
(function testShoe3SolidJoin() {
  ...
  assert(ren.indexOf('clipPtsAtJoinX(fullOuter, 280 - blend, false)') !== -1, 'copper uses continuous U clip');
  assert(ren.indexOf('clipPtsAtJoinX(fullOuter, 280 + blend, true)') !== -1, 'cyan uses continuous U clip');
  assert(ren.indexOf('function extendJoinX') === -1, 'no extendJoinX brick-cut');
  assert(ren.indexOf('if (gmy < 120) return') !== -1, 'arch-end glow scraps deleted');
  assert(ren.indexOf('clipPtsAtJoinX(fullOuter, 280 - blend, false)') !== -1, 'copper uses continuous U clip');
  assert(ren.indexOf('clipPtsAtJoinX(fullOuter, 280 + blend, true)') !== -1, 'cyan uses continuous U clip');
  assert(ren.indexOf('function drawCopperDropMouth') === -1, 'shoe3: drop-mouth scrap stroke deleted');
  assert(ren.indexOf('var dropO') === -1, 'no leftover dropO scrap stroke');
  ...
  assert(idxHtml.indexOf('?v=shoe3') !== -1, 'live cache tag shoe3');
  assert.strictEqual(sim.GRAVITY, 1180);
  assert.strictEqual(sim.TABLE_PITCH_DEG, 6.8);
  assert.strictEqual(sim.HABITRAIL_ASSIST, 0);
  assert.strictEqual(sim.TABLE_W, 560);
  ...
  assert(!right.mergeOuter.some(function (s) { return s.x1 === 526 || s.x2 === 526; }), 'square 526,90 corner deleted');
  assert(right.mergeOuter.some(function (s) { return s.x1 === 524 && s.y1 === 96 && s.x2 === 522 && s.y2 === 84; }), 'copper corner is a rounded turn');
  var right = fresh().sideRoutes.rightRamp;
  assert(right.mergeInner.some(function (s) { return s.x1 === 470 && s.y1 === 88 && s.x2 === 458 && s.y2 === 80; }), 'merge3 plunge start kept');
  assert(right.mergeInner.some(function (s) { return s.x2 === 280 && s.y2 === 80; }), 'join inner y=80 kept');
  assert(!right.mergeInner.some(function (s) { return s.x1 === 372 || s.x2 === 372; }), 'orange inner dent x=372 deleted');
  ...
  console.log('PASS: shoe3 solid U join, no corner scraps, clean copper corner');
})();
