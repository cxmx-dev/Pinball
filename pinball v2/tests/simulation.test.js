...
(function testBoingerCycleAndKick() {
  var state = fresh();
  var b = state.boinger;
  assert(b, 'boinger exists');
  assert.strictEqual(b.x, 318);
  assert.strictEqual(b.y, 686);
  assert(b.radius >= 11 && b.radius <= 13, 'cap radius ~12');
  assert.strictEqual(sim.BOINGER_UP_SEC, 3);
  assert.strictEqual(sim.BOINGER_DOWN_SEC, 1.5);
  sim.stepBoinger(state, 0);
  assert(b.up, 'starts up');
  sim.stepBoinger(state, 2.9);
  assert(b.up, 'still up at 2.9s');
  sim.stepBoinger(state, 0.2);
  assert(!b.up, 'down after 3.1s');
  sim.stepBoinger(state, 1.5);
  assert(b.up, 'up again after full 4.5s cycle');
  state.ball.inPlay = true;
  state.ball.x = b.x;
  state.ball.y = b.y;
  state.ball.vx = 40;
  ...
  assert(state.ball.x === ox && state.ball.vx === ovx, 'pass through when down');
  b.up = true;
  b.pop = 1;
  b.cooldown = 0;
  state.ball.x = b.x - 8;
  ...
  var score0 = state.score;
  sim.collideBoinger(state);
  assert(state.ball.vx < 0, 'kicked back along contact normal (-x)');
  assert(state.score > score0, 'scored a hit');
  console.log('PASS: boinger 3s up / 1.5s down, collide only when up, kick along normal');
})();
