...
(function testPop1TriangleAndDumbbell() {
  ...
  assert(idxSrc.indexOf('?v=shoe2') !== -1, 'cache bust shoe2');
...
(function testShoe2SolidSausageU() {
  var fs = require('fs');
  var path = require('path');
  ...
  assert(ren.indexOf('function drawCopperDropMouth') !== -1, 'copper drop has a mouth');
  ...
  assert(idxHtml.indexOf('?v=shoe2') !== -1, 'live cache tag shoe2');
  assert.strictEqual(sim.GRAVITY, 1180);
  assert.strictEqual(sim.TABLE_PITCH_DEG, 6.8);
  assert.strictEqual(sim.HABITRAIL_ASSIST, 0);
  assert.strictEqual(sim.TABLE_W, 560);
  ...
  console.log('PASS: shoe2 solid cyan/copper sausage U + copper drop + open crown');
})();
